...
(define (extend-key key size)
  (let rec_func ((res '()) (s size) (k key))
    (cond
      ((zero? s) res)
      ((null? k) (rec_func res s key))
      (else (rec_func (append res (list (car k))) (sub1 s) (cdr k)))
      )
    )
  )
...
(define (encrypt-decrypt-numbers op)
  (λ (message key)
    (map (λ (m k)
           (modulo (op m k) 27)
           )
...
(define (encrypt-decrypt-codes op)
  (λ (message key)
    (let ((transform (message->codes message)))
      (codes->message
       (map (λ (m k)
              (modulo (op m k) 27)
              )
            transform
            (extend-key key (length transform))
            )
       )
      )
    )
