...
(define (make-mul-stream Q)
  (stream-append
   (stream (multiply T1 Q))
   (stream (multiply T2 Q))
   (stream (multiply T3 Q))
   )
  )

(define ppt-stream-in-tree-order
  (let rec-func ((Q (stream '(3 4 5))))
    (stream-cons (stream-first Q)
                 (rec-func (stream->list (stream-append
                                          (stream-rest Q)
                                          (make-mul-stream (stream-first Q)))
                                         )
                           )
                 )
    )
...
; H-rest - dupa formarea tuturor perechilor cu un element din H
;          scot din aceasta multime elementul respecitv
; G-reset - se revine la primul element din G dupa ce s-a ajuns
;           la elementele cu acelasi index din ambele multimi
; index-G - memoreaza la al catelea element din multimea G s-a
;           ajuns
; index-G - memoreaza la al catelea element din multimea H s-a
;           ajuns

(define (pairs G H)
  (let rec-cons ([H-rest H] [G-reset G] [index-G 1] [index-H 1])
    (let* ([g (stream-first G-reset)]
           [h (stream-first H-rest)]
           [pair (cons g h)]
           [Hr (stream-rest H-rest)]
           [Gr (stream-rest G-reset)]
           )
      ; daca indecsii sunt egali, in multimea G se trece la primul element
      ; iar in multimea H se merge la urmatorul element
      (if (= index-G index-H)
          (stream-cons pair
                       (rec-cons Hr G 1 (add1 index-H))
                       )
          ; altfel, se continua formarea de perechi cu urmatorul
          ;element din G si acelasi element din H
          (stream-cons pair
                       (rec-cons H-rest Gr (add1 index-G) index-H)
                       )
          )
      )
    )
...
(define (G-H-streams count)
  (stream-cons count (G-H-streams (+ 2 count)))
  )

(define G-stream (G-H-streams 1))
(define H-stream (G-H-streams 3))

(define gh-pairs-stream
  (stream-filter
   (λ (pair)
     (let ([g (car pair)] [h (cdr pair)])
       (and (not (even? g)) (not (even? h))
            (= 1 (gcd g h))
            (< g h)
            )
       )
     )
   (pairs G-stream H-stream)
   )
  )
